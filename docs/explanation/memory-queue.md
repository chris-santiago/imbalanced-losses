...
The weighted arithmetic in `_compute_per_class` only activates when this call supplied a weight or the queue holds at least one row with a stored weight other than `1`; the queue exposes this as `has_weights`, a plain Python `bool`. This is what keeps the unweighted code path bitwise unchanged when `sample_weight` is never used: no weight tensor is ever materialized, and the pre-existing `merge()` call runs exactly as it did before this feature existed.
